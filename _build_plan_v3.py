...
("S2","Frontend","Wire Monitor + Connected + Push to real APIs (remove ALL mock data)","Dev B",18,"Done",1.0,date(2026,6,5),"COMPLETE (T-11) — deleted all 8 mock data files + fallbacks app-wide: Registry/Dashboard/Connected drop the sample fallback (real-or-empty), Monitor run-history -> empty state (real DLQ panel kept; seed/echo demo removed), Alerts -> real /api/alerts, Admin users/clients -> empty, Topbar global search indexes real connectors/connections/entities. Every page shows real backend data or an honest empty state."),
("S2","Vault","Credential decrypt endpoint + DB-connection credential type + test-connection","Dev A",12,"Done",1.0,date(2026,6,5),"COMPLETE (T-12) — VaultPage rewritten onto real /api/credentials (metadata list) + decrypt-on-reveal via /api/credentials/:id/decrypt (10s auto-hide); DB-connection credential form (test + save, AES-256-GCM). No mock keys/owners."),
# Sprint 3
...
("S4","Connectors","12-category connector platform — runtime registry + 8 new runtimes (GraphQL / Flat File / Webhook / Message Queue / File Share / SOAP / Web Scraping / Email)","Dev B",36,"Done",1.0,date(2026,6,5),"COMPLETE — IConnectorRuntime + capabilities; every category is authorable AND moves data. Verified live: GraphQL 250 rec, CSV/XLSX parse, SFTP listing (test.rebex.net), SOAP Add=12, Playwright scrape example.com, Webhook ingest->drain (hub inbox, first prod use), Redis-Streams peek, DB auto-create+upsert. Heavy deps lazy-imported (csv-parse, xlsx, ssh2-sftp-client, strong-soap, imapflow)."),
