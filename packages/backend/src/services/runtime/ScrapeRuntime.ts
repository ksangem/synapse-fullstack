...
export class ScrapeRuntime implements IConnectorRuntime {
  ...
  private async scrape(urls: string[], selectors: Record<string, string>): Promise<Record<string, unknown>[]> {
    ...
    const browser = await chromium.launch({ headless: true }) as {
      newPage(): Promise<{ goto(u: string, o?: unknown): Promise<unknown>; title(): Promise<string>; textContent(s: string): Promise<string | null> }>;
      close(): Promise<void>;
    };
  ...
  async test(creds: Creds, ctx: RuntimeContext): Promise<TestResult> {
    ...
    try {
      const recs = await this.scrape(urls.slice(0, 1), this.selectors(cfg, creds));
      return { ok: true, sampleCount: recs.length, message: `Loaded ${urls[0]}` };
    } catch (e) {
  ...
  async discoverEntities(): Promise<EntitySummary[]> {
    return [{ key: 'page', name: 'Scraped Page', description: 'One record per target URL' }];
  }

  async discoverFields(creds: Creds, ctx: RuntimeContext): Promise<FieldDef[]> {
    const cfg = await this.cfg(ctx);
    return [{ name: 'url', type: 'string' }, { name: 'title', type: 'string' },
      ...Object.keys(this.selectors(cfg, creds)).map((name) => ({ name, type: 'string' }))];
  }
  ...
  async fetch(creds: Creds, _entityKey: string, ctx: RuntimeContext): Promise<FetchResult> {
    ...
    const records = await this.scrape(urls.slice(0, maxPages), this.selectors(cfg, creds));
