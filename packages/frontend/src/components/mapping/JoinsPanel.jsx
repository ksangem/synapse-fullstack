...
const S = {
  ...
  inp: { width: '100%', padding: '6px 8px', fontSize: 'var(--fs-sm)', border: '1px solid var(--border-strong)', borderRadius: 'var(--radius-sm)', background: 'var(--bg-input)', color: 'var(--text)' },
  chip: { display: 'flex', alignItems: 'center', gap: 11, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '11px 13px', marginBottom: 9 },
...
const INTENTS = {
  ...
  enrich:   { icon: '➕', color: 'var(--success-on)',        title: 'Add details',        blurb: 'Pull extra columns from another list or table that shares a key.' },
  aggregate:{ icon: '∑', color: 'var(--warning-on)',        title: 'Summarize',          blurb: 'Count, sum, or average the related records for each item.' },
};
...
function describe(j) {
  ...
  if (intent === 'aggregate') {
    ...
    return { icon: '∑', color: 'var(--warning-on)', text: `Per ${local}: ${parts} from related ${where}` };
  }
  if (intent === 'enrich') {
    const cols = (j.pull || []).map((p) => p.as || p.column).join(', ');
    return { icon: '➕', color: 'var(--success-on)', text: `Add ${cols} from ${where}, matched on ${key}` };
  }
    ...
    <div style={S.panel}>
      ...
      {open && (
        <div style={S.body}>
              ...
              {joins.map((j, i) => {
                ...
                return (
                  <div key={i} style={{ ...S.chip, borderLeft: `3px solid ${d.color}` }}>
                    <span style={{ fontSize: 'var(--fs-md)' }}>{d.icon}</span>
                    <span style={{ fontSize: 'var(--fs-sm)', flex: 1, lineHeight: 1.4 }}>{d.text}</span>
                    <button className="link-btn" onClick={() => startEdit(i)}>Edit</button>
                    <button className="link-btn joins-link-danger" onClick={() => remove(i)}>Remove</button>
                  </div>
                );
              })}
              <button className="btn btn-primary btn-sm" onClick={startAdd}>＋ Add data from another place</button>
            </>
          )}
          ...
          {view === 'intent' && (
            <>
              <p style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-dim)', margin: '2px 0 12px' }}>What do you want to add?</p>
              <div style={S.grid3}>
                {Object.entries(INTENTS).map(([key, it]) => (
                  <button key={key} className="joins-intent" style={S.card(it.color)} onClick={() => chooseIntent(key)}>
                    <div style={{ fontSize: 'var(--fs-lg)', marginBottom: 6 }}>{it.icon} <strong style={{ fontSize: 'var(--fs-md)' }}>{it.title}</strong></div>
                    <div style={{ fontSize: 'var(--fs-sm)', color: 'var(--text-dim)', lineHeight: 1.45 }}>{it.blurb}</div>
                  </button>
                ))}
              </div>
              <div style={{ marginTop: 12, display: 'flex', justifyContent: 'space-between' }}>
                <button className="link-btn" onClick={() => chooseIntent('advanced')}>Advanced (raw editor)…</button>
  ...
  if (manual) {
    return (
      <div style={{ display: 'flex', gap: 6 }}>
        ...
        {known && <button className="btn btn-outline btn-xs" title="Pick from list" onClick={() => { setTyping(false); onChange(''); }}>▾</button>}
      </div>
    );
  }
  return (
...
function GuidedForm({ draft, set, setDraft, localOptions, valid, onSave, onCancel, sides = DEFAULT_SIDES, entitiesFor = () => [], columnsFor = () => [], ensureCols = () => {} }) {
  ...
  const LocalField = (
    <div>
      <label style={S.lbl} htmlFor="joinspanel-which-field-in-your-data-has-the">Which field in your data has the value to match?</label>
      <input id="joinspanel-which-field-in-your-data-has-the" style={S.inp} list={dl} value={d.localField} placeholder="e.g. ClientName" onChange={(e) => set({ localField: e.target.value })} />
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
        <span style={{ fontSize: 'var(--fs-md)' }}>{it ? it.icon : '⚙'}</span>
        <strong style={{ fontSize: 'var(--fs-md)' }}>{it ? it.title : 'Advanced join'}</strong>
      </div>
      ...
      {isAdv ? (
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          ...
          <div>
            <label style={S.lbl} htmlFor="joinspanel-where-does-the-value-live">Where does the value live?</label>
            <select id="joinspanel-where-does-the-value-live" style={S.inp} value={d.side} onChange={(e) => set({ side: e.target.value, ref: '', keyColumn: '', returnColumn: '' })}>
          ...
          <div>
            ...
            {discover && d.ref && tableList.length > 0 && !colList.length && (
              <div style={{ fontSize: 'var(--fs-xs)', color: 'var(--text-dim)', marginTop: 4 }}>loading columns…</div>
            )}
          </div>
          ...
          {d._intent === 'lookup' && (
            <div style={S.grid2}>
              ...
              <div>
                <label style={S.lbl} htmlFor="joinspanel-name-this-result">Name this result</label>
                <input id="joinspanel-name-this-result" style={S.inp} value={d.resultName} placeholder="e.g. account_id" onChange={(e) => set({ resultName: e.target.value })} />
              </div>
            </div>
          )}
          ...
          {d._intent === 'enrich' && (
            <div>
              <label style={S.lbl} htmlFor="joinspanel-columns-to-bring-in">Columns to bring in</label>
              {(d.cols || []).map((c, i) => (
                <div key={i} style={S.rowX}>
                  ...
                  <input id="joinspanel-columns-to-bring-in" style={S.inp} value={c.as} placeholder="name it (e.g. region)" onChange={(e) => set({ cols: d.cols.map((x, j) => j === i ? { ...x, as: e.target.value } : x) })} />
                  <button className="btn btn-outline btn-xs" aria-label="Remove column" onClick={() => set({ cols: d.cols.filter((_, j) => j !== i) })}>×</button>
                </div>
              ))}
              <button className="link-btn" style={{ marginTop: 6 }} onClick={() => set({ cols: [...(d.cols || []), { column: '', as: '' }] })}>+ another column</button>
            </div>
          )}

          {d._intent === 'aggregate' && (
            <div>
              <label style={S.lbl} htmlFor="joinspanel-what-to-calculate">What to calculate</label>
              {(d.aggs || []).map((a, i) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '110px 1fr 1fr 30px', gap: 9, alignItems: 'end', marginTop: 8 }}>
                  <select id="joinspanel-what-to-calculate" style={S.inp} value={a.fn} onChange={(e) => set({ aggs: d.aggs.map((x, j) => j === i ? { ...x, fn: e.target.value } : x) })}>
                  ...
                  <button className="btn btn-outline btn-xs" aria-label="Remove aggregate" onClick={() => set({ aggs: d.aggs.filter((_, j) => j !== i) })}>×</button>
                </div>
              ))}
              <button className="link-btn" style={{ marginTop: 6 }} onClick={() => set({ aggs: [...(d.aggs || []), { fn: 'count', column: '', resultName: '' }] })}>+ another</button>
            </div>
          )}

          <div>
            <button className="link-btn" onClick={() => setAdvOpen(!advOpen)}>{advOpen ? '▾' : '▸'} Advanced options</button>
            {advOpen && (
              <div style={{ ...S.grid2, marginTop: 8 }}>
                <label style={{ fontSize: 'var(--fs-sm)', display: 'flex', alignItems: 'center', gap: 8, color: 'var(--text)' }}>
                  <input type="checkbox" checked={d.op === 'ci-eq'} onChange={(e) => set({ op: e.target.checked ? 'ci-eq' : 'eq' })} />
                  Ignore upper/lower case when matching
                </label>
                {d._intent === 'lookup' && (
                  <div>
                    <label style={S.lbl} htmlFor="joinspanel-if-not-found">If not found</label>
                    <select id="joinspanel-if-not-found" style={S.inp} value={d.onMissing} onChange={(e) => set({ onMissing: e.target.value })}>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
        <button className="btn btn-primary btn-sm" disabled={!valid} onClick={onSave}>Save</button>
        <button className="btn btn-outline btn-sm" onClick={onCancel}>Cancel</button>
        {!isAdv && <button className="link-btn" style={{ marginLeft: 'auto' }} onClick={() => setDraft({ _intent: 'advanced', ...specFromDraft(d, new Set()) })}>Edit as advanced…</button>}
      </div>
    </div>
  );
}
...
function AdvancedFields({ d, set, localOptions }) {
  ...
  return (
    <div style={{ display: 'grid', gap: 10 }}>
      <div style={S.grid3}>
        <div><label style={S.lbl} htmlFor="joinspanel-alias-namespace">Alias (namespace)</label><input id="joinspanel-alias-namespace" style={S.inp} value={d.alias || ''} onChange={(e) => set({ alias: e.target.value.replace(/[^a-zA-Z0-9_]/g, '') })} /></div>
        <div><label style={S.lbl} htmlFor="joinspanel-match-on-this-row-s-field">Match on (this row's field)</label><input id="joinspanel-match-on-this-row-s-field" style={S.inp} list="joins-localopts-adv" value={on.localField} onChange={(e) => setOn({ localField: e.target.value })} />
          <datalist id="joins-localopts-adv">{localOptions.map((n) => <option key={n} value={n} />)}</datalist>
        </div>
        <div><label style={S.lbl} htmlFor="joinspanel-match-type">Match type</label>
          <select id="joinspanel-match-type" style={S.inp} value={on.op || 'eq'} onChange={(e) => setOn({ op: e.target.value })}><option value="eq">exact</option><option value="ci-eq">case-insensitive</option></select>
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '90px 1fr 1fr 100px', gap: 9 }}>
        <div><label style={S.lbl} htmlFor="joinspanel-side">Side</label><select id="joinspanel-side" style={S.inp} value={entity.side} onChange={(e) => setEntity({ side: e.target.value })}><option value="source">source</option><option value="dest">dest</option></select></div>
        <div><label style={S.lbl} htmlFor="joinspanel-field">{entity.side === 'dest' ? 'Table' : 'Entity'}</label><input id="joinspanel-field" style={S.inp} value={entity.ref} onChange={(e) => setEntity({ ref: e.target.value })} /></div>
        <div><label style={S.lbl} htmlFor="joinspanel-key-column">Key column</label><input id="joinspanel-key-column" style={S.inp} value={entity.keyColumn} onChange={(e) => setEntity({ keyColumn: e.target.value })} /></div>
        <div><label style={S.lbl} htmlFor="joinspanel-if-no-match">If no match</label><select id="joinspanel-if-no-match" style={S.inp} value={d.onMissing || 'null'} onChange={(e) => set({ onMissing: e.target.value })}><option value="null">null</option><option value="error">fail row</option></select></div>
      </div>
      <div>
        <label style={S.lbl} htmlFor="joinspanel-pull-columns">Pull columns</label>
        {pull.map((p, i) => (
          <div key={i} style={S.rowX}>
            <input id="joinspanel-pull-columns" style={S.inp} value={p.column} placeholder="column" onChange={(e) => set({ pull: pull.map((x, j) => j === i ? { ...x, column: e.target.value } : x) })} />
            <input style={S.inp} value={p.as} placeholder="as" onChange={(e) => set({ pull: pull.map((x, j) => j === i ? { ...x, as: e.target.value } : x) })} />
            <button className="btn btn-outline btn-xs" aria-label="Remove column" onClick={() => set({ pull: pull.filter((_, j) => j !== i) })}>×</button>
          </div>
        ))}
        <button className="link-btn" style={{ marginTop: 6 }} onClick={() => set({ pull: [...pull, { column: '', as: '' }] })}>+ pull column</button>
      </div>
      <div>
        <label style={S.lbl} htmlFor="joinspanel-aggregates">Aggregates</label>
        {aggregate.map((a, i) => (
          <div key={i} style={{ display: 'grid', gridTemplateColumns: '110px 1fr 1fr 30px', gap: 9, alignItems: 'end', marginTop: 8 }}>
            <select id="joinspanel-aggregates" style={S.inp} value={a.fn} onChange={(e) => set({ aggregate: aggregate.map((x, j) => j === i ? { ...x, fn: e.target.value } : x) })}>{AGG_FNS.map((fn) => <option key={fn} value={fn}>{fn}</option>)}</select>
            ...
            <button className="btn btn-outline btn-xs" aria-label="Remove aggregate" onClick={() => set({ aggregate: aggregate.filter((_, j) => j !== i) })}>×</button>
          </div>
        ))}
        <button className="link-btn" style={{ marginTop: 6 }} onClick={() => set({ aggregate: [...aggregate, { as: '', fn: 'count', column: '' }] })}>+ aggregate</button>
      </div>
    </div>
  );
}
